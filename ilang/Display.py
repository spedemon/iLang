...
class DisplayPylab(): 
    ...
    def plot_variance_roi(self,roi,figure=None,cmap=None,title="Variance ROI",log_scale=False,interpolation='none'): 
    ...
    def plot_covariance_line(self,xy,covariance=None,figure=None,cmap=None,title="Covariance",log_scale=False,interpolation='none'): 
    ...
    def plot_histogram_bivariate(self,coordinates_A,coordinates_B,n_bins=(100,100),range_plot=None,figure=None,cmap=None,title="Histogram",log_scale=False,interpolation='bicubic',subplot=None):       
    ...
    def plot_histogram(self,coordinates,n_bins=100,range_plot=None,figure=None, alpha_line=0.8, alpha_fill=0.2, color_line=None, color_fill=None,title="Histogram",true_value=None,estimated_value=None,subplot=None): 
        ...
        if histogram  is None: 
            ...
            pylab.axvline(x=mean_value, ymin=0.0, ymax=0.99,alpha=alpha_line,color=color_line,linewidth=1,linestyle='dashed')

            if true_value is not None: 
                if not isinstance(true_value,type(1)): 
                    true_value = true_value[coordinates[0],coordinates[1]]
                pylab.axvline(x=true_value, ymin=0.0, ymax=0.99,alpha=alpha_line,color=color_line,linewidth=1,linestyle='solid')
            if estimated_value is not None: 
                if not isinstance(true_value,type(1)): 
                    estimated_value = estimated_value[coordinates[0],coordinates[1]]
                pylab.axvline(x=estimated_value, ymin=0.0, ymax=0.99,alpha=alpha_line,color=color_line,linewidth=1,linestyle='dotted')
            pylab.title(title)
            #pylab.draw()
        return figure

    def plot_histogram_roi_integral(self,roi,n_bins=100,range_plot=None,figure=None, alpha_line=0.8, alpha_fill=0.2, color_line=None, color_fill=None,title="ROI integral",true_value=None,estimated_value=None): 
        ...
        if histogram  is None: 
            ...
            if estimated_value is not None:
                ...
                pylab.axvline(x=estimated_value, ymin=0.0, ymax=0.99,color=color_line,alpha=alpha_line,linewidth=1,linestyle='dashed')
            pylab.title(title)
            #pylab.draw()
        return figure

    def plot_profile(self,axis,index,n_bins=100,range_plot=None,true_value=None,estimated_value=None,figure=None,cmap=None,color_line=None,title="Profile"): 
        #FIXME: implementation is 2-D only, extend to 3-D
        if axis!=0 and axis!=1: 
            raise UnexpectedParameter("axis: 0 or 1")
        if cmap is None:
            cdict = {'red': ((0.0, 1.0, 1.0),(1.0, 0.7, 0.7)),'green': ((0.0, 1.0, 1.0),(1.0, 0.0, 0.0)),'blue': ((0.0, 1.0, 1.0),(1.0, 0.0, 0.0))} 
        ...
        if true_value is not None:
            pylab.plot((n_bins/(range_plot[1]-range_plot[0]))*p_t,color=color_line,linewidth=1,linestyle='solid')
        if estimated_value is not None:
            pylab.plot((n_bins/(range_plot[1]-range_plot[0]))*p_e,color=color_line,linewidth=1,linestyle='dotted')
        pylab.imshow(M.transpose(),cmap=cmap,origin="lower",aspect="auto")
        pylab.title(title)
        #pylab.draw()

    def plot_autocorrelation(self,coordinates, max_lag=None, figure=None, alpha_line=0.8, color_line=None, title="Autocorrelation"): 
    ...
    def plot_acceptance_ratio(self,figure=None, alpha_line=0.8, alpha_fill=0.2, color_line=None, color_fill=None,title="Acceptance Ratio"): 
        return self._plot_simple_curves(self._tracer.get_acceptance_ratio(),figure, alpha_line, alpha_fill, color_line, color_fill,title) 

    def plot_accepted(self,figure=None, alpha_line=0.8, alpha_fill=0.2, color_line=None, color_fill=None,title="Accepted"): 
        return self._plot_simple_curves(self._tracer.get_accepted(),figure, alpha_line, alpha_fill, color_line, color_fill,title) 

    def plot_threshold(self,figure=None, alpha_line=0.8, alpha_fill=0.2, color_line=None, color_fill=None,title="Threshold"): 
        return self._plot_simple_curves(self._tracer.get_threshold(),figure, alpha_line, alpha_fill, color_line, color_fill,title) 

    def _plot_simple_curves(self,curve,figure=None, alpha_line=0.8, alpha_fill=0.2, color_line=None, color_fill=None,title=" "): 
        if color_line  is None: 
            color_line = hsv_to_rgb(0.0,0.85,1)
        if color_fill  is None: 
            color_fill = hsv_to_rgb(0.0,0.85,1)
